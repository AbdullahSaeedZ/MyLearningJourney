...
public:
	...
	void InsertBeforeIndex(int Index, const T& Value)
	{
		InsertAtIndex(Index - 1, Value);
	}

	void InsertAfterIndex(int Index, const T& Value)
	{
		InsertAtIndex(Index + 1, Value);
	}
	...
	{
		InsertAtIndex(_Size, Value); // meaning new item will be in new index (last index + 1)
	}
