...
namespace DataAccessLayer
{
    public class clsPeopleDataAccess
    {
                  ...
                  ref string LastName, ref byte Gender, ref int NationalityCountryID, ref string Phone, ref string Email, ref string Address, ref string ImagePath, ref DateTime BirthDate)
        {
                ...
                using (SqlConnection connection = new SqlConnection(clsDataAccessSettings.connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        if (reader.Read())
                        {
                            ...
                            isFound = true;
                            Log.LogEvent(System.Diagnostics.EventLogEntryType.Information, $"Person data with ID{PersonID} has been fetched");
                        }
                    }
                }    
            }
            ...
            {
                Log.LogEvent(System.Diagnostics.EventLogEntryType.Error, e.Message, e.StackTrace);
                throw new Exception("Error occurred, check logs or contact your Admin.");
            }
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(clsDataAccessSettings.connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        {
                            newID = ID;
                            Log.LogEvent(System.Diagnostics.EventLogEntryType.Information, "New Person Added");
                        }
                    }
                }
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(clsDataAccessSettings.connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        if (rowsAffected > 0) Log.LogEvent(System.Diagnostics.EventLogEntryType.Information, $"Person data with ID{PersonID} has been updated");
                    }
                }
            }
            catch (Exception e)
        ...
        public static bool DeletePerson(int PersonID)
        {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(clsDataAccessSettings.connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        connection.Open();
                        rowsAffected = command.ExecuteNonQuery();
                        if (rowsAffected > 0) Log.LogEvent(System.Diagnostics.EventLogEntryType.Information, $"Person data with ID{PersonID} has been deleted");
                    }
                }
            }
            catch (Exception e)
            {
                Log.LogEvent(System.Diagnostics.EventLogEntryType.Error, e.Message, e.StackTrace);
                throw new Exception("Could not perform process, check logs or contact your Admin.");
                return false;
            }
            return (rowsAffected > 0);
        }
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(clsDataAccessSettings.connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        {
                            dt.Load(reader);
                            Log.LogEvent(System.Diagnostics.EventLogEntryType.Information, $"People records have been fetched");
                        }
                        else
                            dt = null;
                    }
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(clsDataAccessSettings.connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        {
                            isFound = true;
                            Log.LogEvent(System.Diagnostics.EventLogEntryType.Information, $"Person with ID{PersonID} has been queried");
                        }
                    }
                }
            }
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(clsDataAccessSettings.connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        {
                            isFound = true;
                            Log.LogEvent(System.Diagnostics.EventLogEntryType.Information, $"Person with NationalNo {NationalID} has been queried");
                        }
                    }
                }
            }
