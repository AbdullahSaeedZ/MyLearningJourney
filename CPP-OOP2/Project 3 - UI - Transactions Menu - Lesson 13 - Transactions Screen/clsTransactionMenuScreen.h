#pragma once

#include <iostream>
#include "clsScreen.h"
using namespace std;

class clsTransactionMenuScreen : protected clsScreen
{
private:

	enum enTransactionsMenueOptions {
		eDeposit = 1, eWithdraw = 2,
		eShowTotalBalance = 3, eShowMainMenu = 4
	};


	static void _ShowTotalBalancesScreen()
	{
		cout << "Total balances screen..." << endl;
	}

	static void _ShowDepositScreen()
	{
		cout << "Deposit screen here.." << endl;
	}

	static void _ShowWithdrawScreen()
	{
		cout << "Withdraw screen here.." << endl;
	}


	static  void _GoBackToTransactionMenu()
	{
		cout << setw(37) << left << "" << "\n\tPress any key to go back to Transaction Menu...\n";

		system("pause>0");
		ShowTransactionMenu();
	}

	

	static short _ReadTransactionMenuOption()
	{
		short Choice = 0;
		Choice = clsInputValidate::ReadNumInRange(1, 4, "Choose what you want to do [1 to 4]: ");
		return Choice;
	}

	static void _PerformTransactionMenuOption(short Option)
	{
		
		switch (Option)
		{
		case enTransactionsMenueOptions::eShowTotalBalance:
			system("cls");
			_ShowTotalBalancesScreen();
			_GoBackToTransactionMenu();
			break;

		case enTransactionsMenueOptions::eDeposit:
			system("cls");
			_ShowDepositScreen();
			_GoBackToTransactionMenu();
			break;

		case enTransactionsMenueOptions::eWithdraw:
			system("cls");
			_ShowWithdrawScreen();
			_GoBackToTransactionMenu();
			break;

		case enTransactionsMenueOptions::eShowMainMenu:
			system("cls");
			//will exit the menu and go to main menue screen
			break;

		}

	}



public:

	static void ShowTransactionMenu()
	{

		system("cls");
		_DrawScreenHeader("\t  Transactions Screen");

		cout << "===========================================\n";
		cout << "\t\t  Transactions Menu\n";
		cout << "===========================================\n";
		cout << "\t[1] Deposit.\n";
		cout << "\t[2] Withdraw.\n";
		cout << "\t[3] Total Balances.\n";
		cout << "\t[4] Main Menu.\n";
		cout << "===========================================\n";

		_PerformTransactionMenuOption(_ReadTransactionMenuOption());
	}
	

};

